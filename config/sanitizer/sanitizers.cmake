...
if(USE_SANITIZER)
  if(CMAKE_C_COMPILER_ID MATCHES "IntelLLVM" OR CMAKE_C_COMPILER_ID MATCHES "[Cc]lang")
    ...
    if(UNIX)
      append("-fno-omit-frame-pointer" CMAKE_C_SANITIZER_FLAGS)
      message(STATUS "Building with sanitize, base flags=${CMAKE_C_SANITIZER_FLAGS}")

      if(uppercase_CMAKE_BUILD_TYPE STREQUAL "DEBUG")
        append("-O1" CMAKE_C_SANITIZER_FLAGS)
      endif()
      ...
      if(USE_SANITIZER MATCHES "([Aa]ddress);([Uu]ndefined)"
        ...
        append("-fsanitize=address,undefined" CMAKE_C_SANITIZER_FLAGS)
        set(MEMCHECK_TYPE AddressSanitizer)
      elseif(USE_SANITIZER MATCHES "([Aa]ddress)")
        # Optional: -fno-optimize-sibling-calls -fsanitize-address-use-after-scope
        message(STATUS "Building with Address sanitizer")
        append("-fsanitize=address" CMAKE_C_SANITIZER_FLAGS)
        set(MEMCHECK_TYPE AddressSanitizer)
      elseif(USE_SANITIZER MATCHES "([Mm]emory([Ww]ith[Oo]rigins)?)")
        # Optional: -fno-optimize-sibling-calls -fsanitize-memory-track-origins=2
        append("-fsanitize=memory" CMAKE_C_SANITIZER_FLAGS)
        if(USE_SANITIZER MATCHES "([Mm]emory[Ww]ith[Oo]rigins)")
          message(STATUS "Building with MemoryWithOrigins sanitizer")
          append("-fsanitize-memory-track-origins" CMAKE_C_SANITIZER_FLAGS)
        else()
      ...
      elseif(USE_SANITIZER MATCHES "([Uu]ndefined)")
        ...
        append("-fsanitize=undefined" CMAKE_C_SANITIZER_FLAGS)
        if(EXISTS "${BLACKLIST_FILE}")
            append("-fsanitize-blacklist=${BLACKLIST_FILE}" CMAKE_C_SANITIZER_FLAGS)
        endif()
        set(MEMCHECK_TYPE UndefinedBehaviorSanitizer)
      elseif(USE_SANITIZER MATCHES "([Tt]hread)")
        message(STATUS "Building with Thread sanitizer")
        append("-fsanitize=thread" CMAKE_C_SANITIZER_FLAGS)
        set(MEMCHECK_TYPE ThreadSanitizer)
      elseif(USE_SANITIZER MATCHES "([Ll]eak)")
        message(STATUS "Building with Leak sanitizer")
        append("-fsanitize=leak" CMAKE_C_SANITIZER_FLAGS)
