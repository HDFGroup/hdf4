...
foreach (target IN LISTS ${CPACK_EXPORT_LIBRARIES})
    if (WIN32)
        ...
        execute_process(COMMAND $ENV{SIGNTOOLDIR}/signtool
          ...
          $<TARGET_FILE:${target}>
          WORKING_DIRECTORY ${CPACK_TEMPORARY_INSTALL_DIRECTORY}/${CPACK_PACKAGE_INSTALL_DIRECTORY}/bin
        )
    ...
    elseif (APPLE)
        ...
        execute_process(COMMAND codesign
          ...
          $<TARGET_FILE:${target}>
          WORKING_DIRECTORY ${CPACK_TEMPORARY_INSTALL_DIRECTORY}/${CPACK_PACKAGE_INSTALL_DIRECTORY}/lib
        )
        message(STATUS "Signing the target $<TARGET_FILE:${target}>")
    else ()
        set (target_path ${CPACK_TEMPORARY_INSTALL_DIRECTORY}/${CPACK_PACKAGE_INSTALL_DIRECTORY}/lib)
        message(STATUS "Signing the target $<TARGET_FILE:${target}> using ${target_path}")
    endif ()
endforeach ()

foreach (target IN LISTS ${CPACK_EXPORT_UTILS})
    if (WIN32)
        # Sign the targets
        execute_process(COMMAND $ENV{SIGNTOOLDIR}/signtool
          sign /v /debug /fd SHA256 /tr http://timestamp.acs.microsoft.com /td SHA256
    ...
    elseif (APPLE)
        ...
        set (target_path ${CPACK_TEMPORARY_INSTALL_DIRECTORY}/${CPACK_PACKAGE_INSTALL_DIRECTORY}/bin)
