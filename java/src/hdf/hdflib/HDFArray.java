 ...
 * href="./hdf.hdflib.HDFNativeData.html">HDFNativeData</a> class to convert between Java and C
 * representations.
 */
...
public class HDFArray {
        ...
        while (n < ArrayDescriptor.totalSize) {
            ...
            try {
                if (ArrayDescriptor.NT == 'J') {
                    ...
                    HDFJavaException ex =
                        new HDFJavaException("HDFArray: byteify unknown type not implemented?");
                    throw(ex);
                }
    ...
    public Object arrayify(byte[] bytes) throws HDFJavaException
    {
        ...
        if (ArrayDescriptor.dims == 1) {
            ...
            try {
                if (ArrayDescriptor.NT == 'I') {
                    ...
                    HDFJavaException ex =
                        new HDFJavaException("arrayify:  unknown type not implemented yet...");
                    throw(ex);
                }
            }
            catch (OutOfMemoryError err) {
        ...
        default:
            HDFJavaException ex =
                new HDFJavaException("HDFArray: unknown or unsupported type: " + ArrayDescriptor.NT);
            throw(ex);
        ...
        if (ArrayDescriptor.NT != 'B') {
            ...
            if (ArrayDescriptor.currentindex[ArrayDescriptor.dims - 1] !=
                (ArrayDescriptor.dimlen[ArrayDescriptor.dims - 1] - 1)) {
                throw new java.lang.InternalError(
                    new String("HDFArray::arrayify Panic didn't complete all data: currentindex[" + i +
                               "] = " + ArrayDescriptor.currentindex[i] + " (should be " +
                               (ArrayDescriptor.dimlen[i] - 1) + "?"));
