...
if [ "$has32" ]; then
    diff  hdfls.tmp5 hdfimport.out1 || errors=1
    $HDFEDCMD < hdfimport.input1 > hdfed.tmp6 2>&1
    diff  hdfed.tmp6 hdfimport.out2 || errors=1
else
    diff  hdfls.tmp5 hdfimport.out1-32 || errors=1
   $HDFEDCMD < hdfimport.input1-32 > hdfed.tmp6 2>&1
   diff  hdfed.tmp6 hdfimport.out2-32 || errors=1
fi
/bin/rm -f ctxt* cb* *.hdf hdfls.tmp5 hdfed.tmp6
...
if [ $errors -eq 1 ]; then
  ...
  echo " diff hdfls.tmp5 hdfimport.out1-32 "
  echo " $HDFEDCMD < hdfimport.input1-32 >& hdfed.tmp6 "
  echo " diff hdfed.tmp6 hdfimport.out2-32 "
  fi
  echo " ******* END NOTE *************"
  echo ""
  errors=0
