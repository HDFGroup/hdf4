...
objlist_t *make_obj_list(int32 fid,uint32 options)
{
    ...
    for(n=0; n<nobj; n++) {
        ...
        if(options&CHECK_LABEL) { /* are we looking for annotations ? */
            int32 len;
        ...
        if(options&CHECK_DESC) { /* are we looking for annotations ? */
            int32 len;
    ...
    for(n=0,temp_obj = &obj_ret->raw_obj_arr[0]; n<nobj; n++,temp_obj++) {
        if(temp_obj->index==(-1)) { /* first object of this type in the file */
            int32 temp_index;
            ...
            for(m=n, temp2_obj=temp_obj+1; m<nobj; m++,temp2_obj++) {
