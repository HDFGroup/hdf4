 ...
 */
static void pr_vals(vp, len, fmt, more, lastrow, vals) struct ncvar *vp; /* variable */
 ...
 */
static void lastdelim(more, lastrow) bool more;
bool        lastrow;
{
    if (more) {
        Printf(", ");
 ...
 */
static void   annotate(vp, fsp, cor, iel) struct ncvar *vp; /* variable */
 ...
 */
static void pr_cvals(vp, len, fmt, more, lastrow, vals, fsp, cor) struct ncvar *vp; /* variable */
long        len;     /* number of values to print */
char       *fmt;     /*
                      * printf format used for each value.  If
                      * nc_type is NC_CHAR and this is NULL,
