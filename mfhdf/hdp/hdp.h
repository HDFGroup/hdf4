...
typedef struct {
    int help; /* Print help on this command */
} dump_opt_t;
...
typedef struct {
    ...
    int class;                                       /* Whether to dump class information */
    int    name;                                     /* Whether to dump name information */
    int    desc;                                     /* Whether to dump description information */
    int    spec;                                     /* Whether to dump special element information */
    int    group;                                    /* Whether to dump group information */
...
typedef struct {
    ...
    int            *filter_num; /* ref #'s or indices to use as filter */
    ...
    int       keep_order;       /* whether to dump the datasets in the same
                                    order as specified by the user */
    int           dump_to_file; /* whether to dump to a file */
    file_format_t file_format;  /* Is data written in ASCII or binary */
    int           as_stream;    /* whether carriage return added to output data lines */
    int           clean_output; /* whether to print space characters as they
                                    are or to print in \digit format */
    int  firstln_indent;        /* col# where data starts on the first line*/
    int  contln_indent;         /* col# where data continues on the next line*/
    char file_name[MAXFNLEN];   /* Name of file to dump into */
    char ifile_name[MAXFNLEN];  /* Name of input file being processed */

    int            print_pal;     /* for GR only: TRUE if option -p selected */
    gr_interlace_t interlace;     /* user's choice of interlace mode to print data in */
    int            no_lattr_data; /* GR & SD only: TRUE if option -l selected */
    int            no_gattr_data; /* GR & SD only: TRUE if option -g selected */
    int            file_type;     /* netCDF, HDF, or other, which hdp doesn't process */
} dump_info_t;

/* Filename list structure */
typedef struct {
    int    max_files; /* the number of files in the file list */
    int    curr_file; /* the current file */
    char **file_arr;  /* pointer to the filename information */
} filelist_t;

/* Group info structure */
typedef struct {
    int   max_dds; /* the number of DDs in the group */
    int   curr_dd; /* The current DD */
...
typedef struct {
    ...
    unsigned is_group : 1,       /* flag to indicate item is a group */
...
/* DD list structure */
typedef struct {
    int         max_obj;     /* the number of dds in the obj list */
    int         curr_obj;    /* the current obj */
...
extern int   VSattrhdfsize(int32 vsid, int32 findex, int attrindex, int32 *size);
extern int   Vattrhdfsize(int32 vsid, int attrindex, int32 *size);

/* hdp_list.c */
/*extern int print_data_annotations(const char *fname, int32 an_id, ann_type annot_type, uint16 tag, uint16
 * ref);*/
extern int print_all_data_labels(const char *fname, int32 an_id);
extern int print_all_data_descs(const char *fname, int32 an_id);
extern int do_list(int curr_arg, int argc, char *argv[], int help);
extern int print_all_file_labels(const char *fname, int32 an_id);
extern int print_file_descs(const char *f_name, int32 an_id);
/*int print_annots_by_object( const char *fname, int32 an_id, ann_type annot_type, uint16 tag, uint16 ref);
int print_annots_in_file( int32 an_id, char* fname, int32 n_annotations, ann_type annot_type );
*/
int print_all_file_descs(const char *fname, list_info_t *list_opts, /* for print_SDattrs */ int32 an_id);
int print_data_labels(const char *fname, int32 an_id, uint16 tag, uint16 ref);
int print_data_descs(const char *fname, int32 an_id, uint16 tag, uint16 ref);

/* hdp_sds.c */
void  dumpsds_usage(int argc, char *argv[]);
int   parse_dumpsds_opts(dump_info_t *dumpsds_opts, int *curr_arg, int argc, char *argv[]);
int   do_dumpsds(int curr_arg, int argc, char *argv[], int help);
int   dsd(dump_info_t *dumpsds_opts, int curr_arg, int argc, char *argv[]);
...
/* hdp_gr.c */
int do_dumpgr(int curr_arg, int argc, char *argv[], int help);
int parse_dumpgr_opts(dump_info_t *dumpgr_opts, int *curr_arg, int argc, char *argv[]);

/* hdp_dump.c */
extern int   fmtchar(void *x, file_format_t ft, FILE *ofp);
extern int   fmtuchar8(void *x, file_format_t ft, FILE *ofp);
extern int   fmtbyte(unsigned char *x, file_format_t ft, FILE *ofp);
extern int   fmtint(void *x, file_format_t ft, FILE *ofp);
extern int   fmtshort(void *x, file_format_t ft, FILE *ofp);
extern int   fmtint8(void *x, file_format_t ft, FILE *ofp);
extern int   fmtuint8(void *x, file_format_t ft, FILE *ofp);
extern int   fmtint16(void *x, file_format_t ft, FILE *ofp);
extern int   fmtuint16(void *x, file_format_t ft, FILE *ofp);
extern int   fmtint32(void *x, file_format_t ft, FILE *ofp);
extern int   fmtuint32(void *x, file_format_t ft, FILE *ofp);
extern int   fmtfloat32(void *x, file_format_t ft, FILE *ofp);
extern int   fmtfloat64(void *x, file_format_t ft, FILE *ofp);
extern int   dumpfull(int32 nt, dump_info_t *dump_opts, int32 cnt, void *databuf, FILE *ofp, int indent,
                      int cont_indent);
extern int   dumpclean(int32 nt, dump_info_t *dump_opts, int32 cnt, void *databuf, FILE *ofp);
...
extern int   dumpattr(int32 vid, int32 findex, int isvs, file_format_t ft, FILE *fp);
...
extern char *tagnum_to_name(int num);
extern int   tagname_to_num(const char *name);
...
extern filelist_t *make_file_list(int curr_arg, int argc, char *argv[]);
extern char       *get_next_file(filelist_t *f_list, int advance);
...
extern DFdi        *get_next_group(groupinfo_t *g_list, int advance);
...
extern objinfo_t *get_next_obj(objlist_t *o_list, int advance);
extern objinfo_t *goto_nth_obj(objlist_t *o_list, int n);
...
extern int        print_SDattrs(int32 sd_id, FILE *fp, int32 n_file_attrs, dump_info_t *dumpsds_opts);
extern int        print_SDSattrs(int32 sds_id, int32 nattrs, FILE *fp, dump_info_t *dumpsds_opts);

extern int print_GRattrs(int32 gr_id, int32 n_file_attrs, FILE *fp, dump_info_t *dumpgr_opts);
extern int print_RIattrs(int32 ri_id, int ri_index, int32 nattrs, FILE *fp, dump_info_t *dumpgr_opts);
