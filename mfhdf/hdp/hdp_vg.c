...
int32
Vindex_ref(int32 file_id, int32 sear_index)
{
    int32 find_ref  = -1;
    int   index     = 0;
    int   found     = FALSE;
...
void
resetVG(int32 *vg_id, const char *curr_file_name)
{
    if (*vg_id != FAIL) {
        if (FAIL == Vdetach(*vg_id))
...
void
display(vg_info_t *ptr, int32 level,                         /* level of descendants - for indentation */
        vg_info_t **list, int32 num_nodes, int32 root_index, /* index of the node starting the tree */
        int32 firstchild, FILE *fp)
{
    char *name = NULL;
    int   i, k, x, z;
    ...
    *vg_id = Vattach(file_id, vg_ref, "r");
    ...
    if (name_len > 0) {
        *vgname = (char *)malloc(sizeof(char) * (name_len + 1));
        ...
        {
            *n_entries = -1;
            ERROR_GOTO_2("in %s: Vinquire failed for vg ref=%d", "get_VGandInfo", (int)vg_ref);
        }
    }
    else {
    ...
    for (ii = 0; ii < dumpvg_opts->num_chosen; ii++) {
        switch (dumpvg_opts->all_types[ii].type_of_info) {
            ...
            case IS_CLASS:
                ...
                break;
            default:
                fprintf(stderr, "in get_VGindex_list: Info should only be an index, ref number, name, or "
                                "class of a Vgroup\n");
...
/* closeVG combines the processes of Vend, Hclose, freeing the list
   ...
   safe to go on. */
void
closeVG(int32      *file_id,   /* will be returned as a FAIL */
             ...
             int *skipfile)
{
    ...
    if (num_entries != 0) {
        ...
        if (vg_ref == FAIL)
            ERROR_GOTO_2("in %s: VQueryref failed for vgroup with id=%d", "vgBuildGraph", (int)vg_id);

        for (entry_num = 0; entry_num < num_entries; entry_num++) {
            status = Vgettagref(vg_id, entry_num, &elem_tag, &elem_ref);
            if (FAIL == status)
                ERROR_CONT_2("in %s: Vgettagref failed for the %d'th entry", "vgBuildGraph", (int)entry_num);
            ...
            if (elem_ref == 0) /* taken care of ref=0 in tdfr8f and tdf24 for now */
            {
                ...
                else /* VSattach didn't fail */
                {
                    ...
                    if (FAIL == VSgetname(vs, vsname)) {
                        ERROR_NOTIFY_2("in %s: VSgetname failed for vdata with ref#=%d", "vgBuildGraph",
                                       (int)elem_ref);
                    ...
                    if (FAIL == VSdetach(vs))
                        ERROR_NOTIFY_2("in %s: VSdetach failed for vdata with ref#=%d", "vgBuildGraph",
                                       (int)elem_ref);
           ...
           vg_info_t *aNode, int *skipfile)
{
    ...
    if (num_entries != 0) {
        ...
        vg_ref = VQueryref(vg_id);
        if (vg_ref == FAIL)
            ERROR_GOTO_2("in %s: VQueryref failed for vgroup with id=%d", "vgdumpfull", (int)vg_id);
        ...
        for (entry_num = 0; entry_num < num_entries; entry_num++) {
            status = Vgettagref(vg_id, entry_num, &elem_tag, &elem_ref);
            if (FAIL == status)
                ERROR_CONT_2("in %s: Vgettagref failed for the %d'th entry", "vgdumpfull", (int)entry_num);
            ...
            if (elem_ref == 0) /* taken care of ref=0 in tdfr8f and tdf24 for now */
            {
                ...
                if (vsize == FAIL)
                    ERROR_NOTIFY_2("in %s: VShdfsize failed for vdata with ref#=%d", "vgdumpfull",
                                   (int)elem_ref);
                ...
                if (FAIL == VSdetach(vs))
                    ERROR_NOTIFY_2("in %s: VSdetach failed for vdata with ref#=%d", "vgdumpfull",
                                   (int)elem_ref);
...
dvg(dump_info_t *dumpvg_opts, int curr_arg, int argc, char *argv[])
{
    ...
    while (curr_arg < argc) {
        ...
        if (status == FAIL)
            ERROR_CONT_1("in dvg: print_file_annotations failed for file %s", file_name);
        ...
        curr_vg = 0;
        while ((vg_ref != FAIL) && (dumpall || vg_count < num_vg_chosen))

        {
            ...
            if (!skipvg) {
                if (FAIL == (vg_tag = VQuerytag(vg_id)))
                    ERROR_NOTIFY_3("in dvg: %s failed on vgroup with ref=%d in file %s", "VQuerytag",
            ...
            if (skipvg || dumpvg_opts->contents == DHEADER) {
                ...
                if (status == FAIL)
                    ERROR_NOTIFY_3("in dvg: %s failed for vgroup with ref#=%d in file %s", "vgBuildGraph",
                                   (int)vg_ref, file_name);
            }
            else {
                fprintf(fp, "Entries:-\n");
            ...
            if (skipfile)
                ERROR_BREAK_1("in dvg: Severe failure in file %s.  Go to next file", file_name, FAIL);
