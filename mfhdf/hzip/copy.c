...
comp_t* get_comp(options_t *options, char* path);
													...
													table_t *table)
{
	...
	VOIDP            dim_buf=NULL;
	int              i, j, ret=1;
	comp_t          *comp=NULL;      /* compression info got from table */
	...
	*/
    ...
    chunk_flags = HDF_NONE;
			...
			else
			{
				chunk_flags = HDF_CHUNK;
				for (j = 0; j < rank; j++) 
					chunk_def.chunk_lengths[j] = options->chunk_g.chunk_lengths[j];
			}
		 ...
		 if (obj!=NULL)
			{
				...
				else if (obj->chunk.rank>0)
				{
					...
					chunk_flags = HDF_CHUNK;
					for (j = 0; j < rank; j++) 
						chunk_def.chunk_lengths[j] = obj->chunk.chunk_lengths[j];
													...
													table_t *table)
{
	...
	if ( options->trip==0 ) {
		...
		/* find out what to do with this object */
/*	comp=get_comp(options,path); */
