...
void hzip_addcomp(char* str, options_t *options)
{
	...
	comp_t          comp;               /*compression info for the current -t option entry */
	int             n_objs;             /*number of objects in the current -t or -c option entry */
	int             ret, i;

	comp_info_t     comp1;              /*compression info for the current -t option entry */

	
	/* parse the -t option */
	obj_list=parse_comp(str,&n_objs,&comp);

	comp1.info=comp.info;
  ...
  options_add_comp(obj_list,n_objs,comp1,options->op_tbl);
