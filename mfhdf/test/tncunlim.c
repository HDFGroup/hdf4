    ...
    { /* Read data of the variables */
        ...
        for (ii = 0; ii < edges[0]; ii++) {
            if (outdata[ii] != result1[ii])
                fprintf(
                    stderr,
                    "test_1dim_multivars: Read data %d doesn't match input %d at index %d in Variable 1\n",
        ...
        for (ii = 0; ii < edges[0]; ii++) {
            if (outdata[ii] != result2[ii])
                fprintf(
                    stderr,
                    "test_1dim_multivars: Read data %d doesn't match input %d at index %d in Variable 2\n",
...
test_multidim_singlevar()
{
    ...
    /* result data to compare against read data; the first two elements will
        be changed to "1,2" later for the last test. */
    short result[DIM0][DIM1][DIM2] = {300, -3, -3, -3, -3,  -3, 301, -3,  -3,  -3,  -3,  -3, 302, -3,
                                      -3,  -3, -3, -3, 303, -3, -3,  -3,  -3,  -3,  -3,  -3, -3,  -3,
                                      -3,  -3, -3, -3, -3,  -3, -3,  -3,  400, -3,  -3,  -3, -3,  -3,
                                      401, -3, -3, -3, -3,  -3, 800, 801, 802, 803, 804, 805};
            ...
            for (jj = 0; jj < DIM1; jj++)
                for (kk = 0; kk < DIM2; kk++)
                    if (outdata[ii][jj][kk] != result[ii][jj][kk])
                        fprintf(stderr,
                                "test_multidim_singlevar: Read data %d doesn't match input %d at indices "
                                "%d,%d,%d\n",
