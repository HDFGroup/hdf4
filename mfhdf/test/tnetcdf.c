...
test_read_dim()
{
    int32       fid = FAIL, sds_id = FAIL, status, dim1_id = FAIL;
...
static int16 netcdf_u16[2][3] = {{1, 2, 3}, {4, 5, 6}};
...
test_netcdf_reading()
{
    ...
    if (n_datasets != 8) {
        fprintf(stderr,
                "netCDF Read Test 1: SDfileinfo returned wrong number of datasets in file test1.nc \n");
        num_errs++;
    }

    if (n_file_attrs != 1) {
        fprintf(stderr, "netCDF Read Test 1: SDfileinfo returned wrong number of file attributes in file "
                        "test1.nc \n");
        num_errs++;
    }
    ...
    for (index = 0; index < n_datasets; index++) {
        ...
        if (rank == 2 && num_type == (int32)DFNT_INT16) { /* should only be one of these */
            start[0] = start[1] = 0;
            edges[0]            = dim_sizes[0];
            edges[1]            = dim_sizes[1];
            ...
            for (j = 0; j < dim_sizes[0]; j++) {
                for (i = 0; i < dim_sizes[1]; i++) {
                    if (array_data[j][i] != netcdf_u16[j][i]) {
                        fprintf(stderr,
                                "netCDF Read Test 1: bogus val read: wanted netcdf[%d][%d]=%d, read "
                                "array[%d][%d]=%d \n",
        ...
        ENDSDS(sds_id, "netCDF Read Test 1. SDendaccess failed for dataset in file test1.nc");
    } /* end querying every dataset in file */

    /* Terminate access to the SD interface and close the file. */
    ENDSD(sd_id, "netCDF Read Test 1. SDend failed for file test1.nc");
