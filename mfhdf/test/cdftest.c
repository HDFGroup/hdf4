...
static void  chkgot(type, got, check) nc_type type;
union getret got;
double       check;
{
    switch (type) {
...
static void createtestdims(cdfid, num_dims, sizes, dim_names) int cdfid;
int         num_dims;
long       *sizes;
...
static void testdims(cdfid, num_dims, sizes, dim_names) int cdfid;
int         num_dims;
long       *sizes;
const char *dim_names[];
{
...
static void     createtestvars(id, testvars, count) int id;
...
static void parray(label, count, array) char *label;
unsigned    count;
long        array[];
{
...
static void fill_seq(id) int id;
{
...
static void check_fill_seq(id) int id;
{
    long         vindices[NUM_DIMS];
    long        *cc, *mm;
    union getret got;
    ...
    for (ii = 0; ii < cdesc->num_vars; ii++, tvp++) {
        ...
        for (jj = 0; jj < vdesc->ndims; jj++) {
            if (tvp->dims[jj] != vdesc->dims[jj]) {
                printf("inconsistant dim[%d] for variable %d: %d != %d\n", (int)jj, (int)ii,
