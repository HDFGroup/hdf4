 ...
 */
void test_ncvardef(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs = 0;
    int                  cdfid; /* netcdf id */
    ...
    static struct cdfvar va[] = {/* variables of all shapes and sizes */
                                 {"bytev", NC_BYTE, 6, ___, 0},    {"charv", NC_CHAR, 5, ___, 0},
                                 {"shortv", NC_SHORT, 4, ___, 0},  {"longv", NC_LONG, 3, ___, 0},
                                 {"floatv", NC_FLOAT, 2, ___, 0},  {"doublev", NC_DOUBLE, 1, ___, 0},
    ...
    if (ncvardef(cdfid, va[0].name, va[0].type, va[0].ndims, va[0].dims) != -1) {
    ...
    for (id = 0; id < nd; id++) {
        if ((di_id[id] = ncdimdef(cdfid, di[id].name, di[id].size)) == -1) {
            error("%s: ncdimdef failed for %s, size %d", pname, di[id].name, di[id].size);
            ncclose(cdfid);
            return;
        }
    ...
    for (iv = 0; iv < nv; iv++) {
        ...
        if (strcmp(tmp.name, va[iv].name) != 0 || tmp.type != va[iv].type || tmp.ndims != va[iv].ndims ||
    ...
    if (ncvardef(cdfid, "baddims", va[0].type, va[0].ndims, va[0].dims) != -1) {
        error("%s: ncvardef should have failed on negative dim id in dims", pname);
        ncclose(cdfid);
        return;
    }
    if (ncendef(cdfid) == -1) {
