 ...
 */
void test_ncvarid(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    for (id = 0; id < xx.ndims; id++)
        xx.dims[id] = id;
    if ((varid = ncvardef(cdfid, xx.name, xx.type, xx.ndims, xx.dims)) == -1) {
 ...
 */
void test_ncvarinq(path) char *path; /* name of writable netcdf file to open */
{
    int           nerrs   = 0;
            ...
            switch (test.vars[iv].type) { /* compare values of right type */
                ...
                case NC_CHAR:
                    if (elm[ie].val.by != chval) {
                        error("%s: ncvarget1 returned char %d, expected %d", pname, chval, elm[ie].val.by);
                        nerrs++;
                    }
                    break;
                case NC_SHORT:
                    if (elm[ie].val.sh != shval) {
                        error("%s: ncvarget1 returned short %d, expected %d", pname, shval, elm[ie].val.sh);
                        nerrs++;
                    }
                    break;
                case NC_LONG:
                    if (elm[ie].val.lo != loval) {
                        error("%s: ncvarget1 returned long %ld, expected %d", pname, loval, elm[ie].val.lo);
                        nerrs++;
                    }
                    break;
                case NC_FLOAT:
 ...
 *    try with bad netCDF handle, check error
 */
void test_ncvarput1(path) char *path; /* name of writable netcdf file to open */
{
    int           nerrs   = 0;
 ...
 *    try with bad netCDF handle, check error
 */
void test_ncvarget1(path) char *path; /* name of writable netcdf file to open */
{
    int           nerrs   = 0;
 ...
 */
void test_ncvarrename(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    for (id = 0; id < yy.ndims; id++)
        yy.dims[id] = id;
    if ((yy_id = ncvardef(cdfid, yy.name, yy.type, yy.ndims, yy.dims)) == -1) {
        error("%s: ncvardef failed", pname);
        ncclose(cdfid);
        return;
    }
    ...
    var.dims = (int *)emalloc(sizeof(int) * H4_MAX_VAR_DIMS);
    var.name = (char *)emalloc(H4_MAX_NC_NAME);
    if (ncvarinq(cdfid, yy_id, var.name, &var.type, &var.ndims, var.dims, &var.natts) == -1) {
    ...
    if (strcmp(var.name, newname) != 0) {
        error("%s: ncvarrename changed name to %s instead of %s", pname, var.name, newname);
        ncclose(cdfid);
        return;
    }
