 ...
 */
void test_nccreate(path) char *path; /* name of cdf file to create */
{
    int         nerrs   = 0;
 ...
 */
void test_ncopen(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    if ((cdfid0 = ncopen(xpath, NC_WRITE)) != -1) {
        error("%s: ncopen should fail writing nonexistent file", pname);
        return;
    }
    ...
    /* in define mode */
    if (ncattput(cdfid0, NC_GLOBAL, "title", NC_CHAR, title.len, title.val) == -1) {
    ...
    if ((cdfid1 = ncopen(path, NC_WRITE)) == -1) {
        ...
        /* second open OK */
        if (cdfid0 == cdfid1) {
            error("%s: ncopen should return new cdfid on second open", pname);
            nerrs++;
        }
        if (ncclose(cdfid1) == -1) {
 ...
 */
void test_ncredef(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    aa.dims[0] = ii_dim;
    if ((aa_id = ncvardef(cdfid, aa.name, aa.type, aa.ndims, aa.dims)) == -1) {
    ...
    aa_units.var = aa_id;
    if (ncattput(cdfid, aa_units.var, aa_units.name, aa_units.type, aa_units.len, (void *)aa_units.val) ==
        -1) {
        error("%s: ncattput failed", pname);
        ncclose(cdfid);
        return;
 ...
 */
void test_ncendef(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    bb.dims[0] = kk_dim;
    bb.dims[1] = jj_dim;
    if ((bb_id = ncvardef(cdfid, bb.name, bb.type, bb.ndims, bb.dims)) == -1) {
        error("%s: ncvardef failed", pname);
        ncclose(cdfid);
        return;
    }
 ...
 *  On exit netcdf files are closed.
 */
void test_ncclose(path) char *path; /* name of writable netcdf file to open */
{
    int         nerrs   = 0;
 ...
 *  On exit netcdf files are closed.
 */
void test_ncinquire(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    /* add dimensions */
    for (id = 0; id < nd; id++) {
        if ((dimids[id] = ncdimdef(cdfid, dims[id].name, dims[id].size)) == -1) {
    ...
    for (iv = 0; iv < nv; iv++) {
        ...
        if ((cc_id = ncvardef(cdfid, cc[iv].name, cc[iv].type, cc[iv].ndims, cc[iv].dims)) == -1) {
            error("%s: ncvardef failed", pname);
            ncclose(cdfid);
            return;
        }
 ...
 *  On exit netcdf files are closed.
 */
void test_ncsync(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
    ...
    /* put some data into a variable */
    {
        ...
        if ((cdfid1 = ncopen(path, NC_NOWRITE)) == -1) {
            ...
            if (cdfid0 == cdfid1) {
                error("%s: second ncopen should return distinct handle", pname);
                nerrs++;
            } /* read data just put after a sync, should succeed */
            if (ncvarget(cdfid1, dd_id, dd_start, dd_edges, (void *)got_vals) == -1) {
                error("%s: ncvarget failed", pname);
                nerrs++;
            }
            if (dd_vals[0] != got_vals[0] || dd_vals[1] != got_vals[1]) {
                error("%s: ncvarget succeeded but data values wrong", pname);
            }
            if (ncclose(cdfid1) == -1) {
                error("%s: ncclose failed", pname);
                nerrs++;
 ...
 *  On exit netcdf files are closed.
 */
void test_ncabort(path) char *path; /* name of writable netcdf file to open */
{
    int                  nerrs   = 0;
