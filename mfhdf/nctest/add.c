...
void add_dim(test, idim) /* add the dimension idim to the netcdf test */
    struct netcdf *test;
struct cdfdim *idim;
{
...
void add_var(test, ivar) /* add the variable ivar to the netcdf test */
    struct netcdf *test;
struct cdfvar *ivar;
{
...
void add_att(test, varid, iatt) /* add attribute iatt to the netcdf test */
    struct netcdf *test;
...
void add_reset(test) /* reset in-memory netcdf test to empty */
    struct netcdf *test;
{
    test->ndims  = 0;
    test->nvars  = 0;
...
void del_att(test, varid, iatt) /* delete attribute iatt in the netcdf test */
    struct netcdf *test;
int            varid; /* variable id */
struct cdfatt *iatt;
{
...
void add_data(test, varid, start, edges) /* keep max record written updated */
    struct netcdf *test;
int  varid;
long start[];
long edges[];
...
void           errvar(cdfp, varp) struct netcdf *cdfp;
struct cdfvar *varp;
{
    const char *types;
    int         id;
