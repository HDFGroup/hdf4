 ...
 */
int        arrayp(label, count, array) const char *label;
unsigned   count;
const long array[];
{
 ...
 */
static bool_t nc_API(caller) const char *caller;
{
    char *nc_api = NULL;
...
NC_var *vp;
{
    ...
    if (handle->hdf_mode == DFACC_RDONLY) { /* yes, not good */
        ret_value = DFREF_NONE;
        goto done;
    }
    ...
    if (IS_RECVAR(vp)) {
        ...
        vp->aid = HLcreate(handle->hdf_file, DATA_TAG, vsid, block_size, BLOCK_COUNT);
        if (vp->aid == FAIL) {
            ret_value = DFREF_NONE;
            goto done;
        }
...
hdf_xdr_NCvdata(NC *handle, NC_var *vp, u_long where, nc_type type, uint32 count, void *values)
{
    ...
    */
    ...
    if (elem_length <= 0 && where > 0) { /* fill in the lead sequence of bytes with the fill values */
        ...
        else { /* don't write fill values, just seek to the correct location */
    ...
    if (handle->xdrs->x_op == XDR_DECODE) /* the read case */
    {
        ...
        else {            /* no convert, write directly from the user's buffer */
    ...
    if (elem_length <= 0 && bytes_left > 0) {
        if ((handle->flags & NC_NOFILL) == 0 || isspecial == SPECIAL_COMP) {
          ...
          vp->szof, i.e., before conversion */
            ...
            /* Fill the temporary buffer tBuf with the fill-value specified                    in the
             * attribute if one exists, otherwise, with the default value */
            if (attr != NULL)
                HDmemfill(tBuf, (*attr)->data->values, vp->szof, fill_count);
            else
                NC_arrayfill(tBuf, tempbuf_size, vp->type);
...
Void       *values;
{
     ...
     */
    if (*edges <= 0) {
        NCadvise(NC_EINVALCOORDS, "%s: Invalid edge length %ld", vp->name->values, *edges);
        return -1;
    }
 ...
 */
int
            NC_fill_buffer(handle, varid, edges, values)
