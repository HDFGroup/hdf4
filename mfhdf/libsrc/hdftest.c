...
*/
    ...
    */
    ...
    if(newsds4 == FAIL) 
      {
        fprintf(stderr, "Failed to create a new 3D float32 data set \n");
        num_err++;
      }
    ...
    if(newsds5 == FAIL) 
      {
        fprintf(stderr, "Failed to set a new uint16 3D data set chunked\n");
        num_err++;
      }
    ...
    if ((index = SDnametoindex(f1,"DataSetChunked_3D_SKIP_HUF_2")) == FAIL)
        {
            fprintf(stderr, "Failed to re-select new chunked, Skipping Huffman compressed data set\n");
            num_err++;
        }

    newsds6 = SDselect(f1,index);
