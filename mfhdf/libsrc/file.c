 ...
 */
static int NC_open(path, mode) const char *path; /* file name */
int        mode;
{
    NC  *handle;
    int  cdfid;
...
int nccreate(path, cmode) const char *path; /* file name */
int cmode;
{
...
int ncopen(path, mode) const char *path; /* file name */
int mode;
{
 ...
 */
static char *NCtempname(proto) const char *proto;
{
...
int cdfid;
{
    ...
    if (handle->flags & NC_INDEF) /* in define mode already */
    {
        ...
        if (stash)
            NCadvise(NC_EINDEFINE, "%s: in define mode aleady", stash->path);
        return (-1);
    }
    if (!(handle->flags & NC_RDWR)) {
 ...
 */
static void NC_begins(handle) NC *handle;
{
    unsigned ii;
    u_long   index = 0;
    NC_var **vpp;
    ...
    for (ii = 0; ii < handle->vars->count; ii++, vpp++) {
        ...
        index += (*vpp)->len;
#ifdef EDEBUG
        NCadvise(NC_NOERR, "%s pass 1 begin %d, length %d", (*vpp)->name->values, (*vpp)->begin, (*vpp)->len);
#endif /* EDEBUG */
    }
    ...
    for (ii = 0; ii < handle->vars->count; ii++, vpp++) {
        ...
        (*vpp)->begin = index;
#ifdef EDEBUG
        NCadvise(NC_NOERR, "%s pass 2 begin %d, len %d, *dsizes %d", (*vpp)->name->values, (*vpp)->begin,
                 index, (*vpp)->len, *(*vpp)->dsizes);
#endif /* EDEBUG */
        index += (*vpp)->len;
...
#ifdef EDEBUG
    NCadvise(NC_NOERR, "begin %d, recsize %d, numrecs %d", handle->begin_rec, handle->recsize,
             handle->numrecs);
#endif /* EDEBUG */
