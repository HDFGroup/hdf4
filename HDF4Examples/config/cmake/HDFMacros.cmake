...
macro (SET_HDF_OUTPUT_DIRS package_prefix)
  if (NOT ${package_prefix}_EXTERNALLY_CONFIGURED)
    ...
    if(_isMultiConfig)
      set (CMAKE_TEST_OUTPUT_DIRECTORY ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${H4EX_CFG_NAME})
...
macro (SET_HDF_BUILD_TYPE)
  get_property (_isMultiConfig GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG)
  if (_isMultiConfig)
    # H4EX_CFG_BUILD_TYPE is used in the Fortran install commands for the build location of the .mod files
    set (H4EX_CFG_BUILD_TYPE \${CMAKE_INSTALL_CONFIG_NAME})
    if (CMAKE_BUILD_TYPE)
      # set the default to the specified command line define
      set (H4EX_CFG_NAME ${CMAKE_BUILD_TYPE})
    else ()
      # set the default to the MultiConfig variable
      set (H4EX_CFG_NAME "$<CONFIG>")
    endif ()
  else ()
    set (H4EX_CFG_BUILD_TYPE ".")
    if (CMAKE_BUILD_TYPE)
      set (H4EX_CFG_NAME ${CMAKE_BUILD_TYPE})
    else ()
      set (H4EX_CFG_NAME "Release")
    endif ()
  endif ()
endmacro ()
...
macro (HDF_DIR_PATHS package_prefix)
  option (H4EX_USE_GNU_DIRS "ON to use GNU Coding Standard install directory variables, OFF to use historical settings" OFF)
