...
public class JHVImageAnimation {
    ...
    // new instance using for HDF image animation 
    public JHVImageAnimation(String  file) {
	...
	if ((file != null) && (file.length()!=0) ) { // valid HDF file
	   ...
	   // using GR interface, need to fix bug, which is to read an old 
   	   // HDF file created by DFR8* with a compression. 
	   // int imageNumber = getHDFGRImageNumber(fid);
	   // retrieve a raster image number
	   
	   try {
	   int imageNumber = 0 ;
	   imageNumber = hdf.DFR8nimages(file);

	   // variable to keep image info.
	   AnimatedImageInfo imageInfos[] = new AnimatedImageInfo[imageNumber];

	   // retrieve images info.
	   // setHDFGRImagesInfo(fid, imageInfos);
	   setHDFRaster8Info(file, imageInfos);
...
throws HDFException {
    ...
    public void setHDFGRImagesInfo(int fid, AnimatedImageInfo[] infos) {
	...
	for (int kk=0; kk<infos.length; kk++) {
		   ...
		   if (hdf.GRgetiminfo(riid, name, imgInfo, dim_sizes) ) {
			...
			if (image != null)
			   infos[kk] = new AnimatedImageInfo(image, name[0]);
    ...
    // retrieve raster image(s) and "name" , then hold it for animation
    public void setHDFRaster8Info(String filename, AnimatedImageInfo[] infos) {
	...
	for (int kk=0; kk<infos.length; kk++) {
		   ...
		   // raster image name
		   String name = "Raster Image #" + kk;

		   int[] imgSizes = new int[2];   
		   boolean pals[] = new boolean[1];
   
    		  // get HDF image information
    		  if (hdf.DFR8getdims(filename,imgSizes,pals) == false)
      			return;
   
		  int w = imgSizes[0];
    		  int h = imgSizes[1];
    		  boolean hasPalette = pals[0];
 
  		  byte imageData[]    = new byte[w*h];
    		  byte imagePalette[] = new byte[256*3];
     
    		  // read the image
    		  boolean readFlag = hdf.DFR8getimage(filename, imageData,w,h,imagePalette);
  		  // set image palette
    		  if (hasPalette == false) {
      		     for (int i=0; i<256; i++)
			for (int j=0; j<3; j++)  
 		  ...
 		  Image image=null;
    		  if (readFlag) 
      			image = createRasterImage(imageData,w,h,imagePalette);
