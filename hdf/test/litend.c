...
test_little_read(void)
{
    ...
    if (dimsizes[0] != CDIM_Y || dimsizes[1] != CDIM_X) {
        ...
        if (numbertype != DFNT_LFLOAT64) {
            ...
            if (Verbocity > 9) {
                intn   i;
                uint8 *u8_s = (uint8 *)cdata_f64, *u8_d2 = (uint8 *)data_f64;
            ...
            {
                intn     i;
                float64 *cd_f64 = (float64 *)cdata_f64, *d_f64 = (float64 *)data_f64;

                for (i = 0; i < CDIM_X * CDIM_Y; i++) {
                    if (d_f64[i] < (cd_f64[i] - FLOAT64_FUDGE) || d_f64[i] > (cd_f64[i] + FLOAT64_FUDGE)) {
                        fprintf(stderr, "FLOAT64 data was incorrect\n");
                        printf("cd_f64[%d]=%lf, d_f64[%d]=%lf\n", i, cd_f64[i], i, d_f64[i]);
                        {
                            intn   j;
                            uint8 *u8_s = (uint8 *)&cd_f64[i], *u8_d2 = (uint8 *)&d_f64[i];
