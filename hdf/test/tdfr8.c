...
test_GRgetcomptype()
{
    ...
    int32        file_id = FAIL, gr_id = FAIL, ri_id = FAIL;
    ...
    int maplist[N_IMAGES] = {TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, TRUE, TRUE};
    ...
    CHECK(status, FAIL, "DFR8putimage");
    ...
    CHECK(status, FAIL, "DFR8setcompress");
    ...
    for (ii = 0; ii < num_images; ii++) {
        ...
        CHECK(status, FAIL, "GRgetcomptype");
        ...
        VERIFY(is_mappedable, maplist[ii], "GR2bmapped");
        VERIFY(name_generated, TRUE, "GR2bmapped");

        /* Verify compression type */
        if (comp_type != check_comp[ii])
            VERIFY(check_comp[ii], comp_type, "GRgetcomptype");
