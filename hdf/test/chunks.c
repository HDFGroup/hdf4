...
test_chunks(void)
{
    ...
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 1, (int)chunk[0].pdims[1].dim_length, 1,
                      (int)chunk[0].pdims[1].chunk_length););
    ...
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 1, (int)chunk[0].pdims[1].dim_length, 1,
                      (int)chunk[0].pdims[1].chunk_length););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 2, (int)chunk[0].pdims[2].dim_length, 2,
                      (int)chunk[0].pdims[2].chunk_length););
    ...
    /* check chunk_lengths */
    if (info_block.cdims != NULL) {
        if ((info_block.cdims[0] != 2) || (info_block.cdims[1] != 3) || (info_block.cdims[2] != 4)) {
            fprintf(stderr, "ERROR: HDget_specail_info does not return the correct values \n");
            errors++;
            goto done;
        }
    ...
    /* check chunk_lengths */
    if (info_block.cdims != NULL) {
        if ((info_block.cdims[0] != 1) || (info_block.cdims[1] != 1) || (info_block.cdims[2] != 4)) {
            fprintf(stderr, "ERROR: HDget_specail_info does not return the correct values \n");
            errors++;
            goto done;
        }
    ...
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 2, (int)chunk[0].pdims[2].dim_length, 2,
                      (int)chunk[0].pdims[2].chunk_length););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 3, (int)chunk[0].pdims[3].dim_length, 3,
                      (int)chunk[0].pdims[3].chunk_length););
    ...
    MESSAGE(5,
            printf("Test 11. Create another new element as a 2-D, uint8 chunked, RLE Compressed element\n"););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 0, (int)chunk[0].pdims[0].dim_length, 0,
                      (int)chunk[0].pdims[0].chunk_length););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 1, (int)chunk[0].pdims[1].dim_length, 1,
                      (int)chunk[0].pdims[1].chunk_length););
    ...
    fid = Hopen(TESTFILE_NAME, DFACC_RDWR, 0);
    CHECK(fid, FAIL, "Hopen");
    MESSAGE(5, printf("Test 12. Create another new element as a 3-D, uint8 chunked, GZIP Compressed "
                      "element(192 bytes)\n"););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 0, (int)chunk[0].pdims[0].dim_length, 0,
                      (int)chunk[0].pdims[0].chunk_length););
    MESSAGE(5, printf(" dim_length[%d]=%d, chunk_length[%d]=%d \n", 1, (int)chunk[0].pdims[1].dim_length, 1,
    ...
    MESSAGE(5,
            printf("Verifying 24 bytes from uint8 chunked, GZIP Compressed element using HMCreadChunk\n"););
    /* read data back as chunks */
    dims[0] = 0;
    dims[1] = 0;
    dims[2] = 0;
    ...
    /* Frees dimensions allocate for chunk definition and I/O buffers */
    free(chunk[0].pdims);
    free(outbuf);
    free(inbuf);
