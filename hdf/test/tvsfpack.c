...
fpack(void)
{
    ...
    rec_size = 2 * sizeof(float) + sizeof(int16) + sizeof(char);

    databuf = (uint8 *)malloc(((2 * sizeof(float)) + sizeof(int16) + sizeof(char)) * NRECORDS);
...
funpack(void)
{
    ...
    for (i = 0; i < NRECORDS; i++)
        if ((iident[i] != (char)('A' + i)) ||
            (fabs((double)(itemp[i] - (float)1.11 * (float)(i + 1))) > EPS) || (ispeed[i] != i) ||
    ...
    for (i = 0; i < NRECORDS; i++)
        if ((iident[i] != (char)('a' + i)) ||
            (fabs((double)(itemp[i] - (float)3.33 * (float)(i + 1))) > EPS) || (ispeed[i] != 2 * i) ||
