...
*/
     /*
       ...
       It shows how data can be stored in vdatas, and how vgroups can be 
       created from vdatas, vgroups and other HDF elements.
       ...
       float type. The routine VHstoredata() stores the pressure values in 
       a vdata.
       
       Color data comprises 3 components (red, green, blue). These can be 
       stored as 3 different fields in 3 vdatas. But this example shows 
       that they can be treated as ONE field, and stored together as a 
       compound field called "COLOR".  The number of components of a field 
       is called its order.  In this case, "COLOR" has order=3. 
       (whereas "PRES" above has order=1).
       
       Note that VHstoredatam() must be used to store values of a compound 
       field.  This is similar to VHstoredata() but has an extra argument 
       for spefcifying the order.
       
       Finally a vgroup is created, and the ids of the created vdatas 
       are stored in the vgroup. This effectively links the vdatas 
       together. This example also shows that you can insert another 
       vgroup, as well as a non-vset element  (in this case, some 
       element with tag=7777 and ref=1) into a vgroup.
*/
...
doit() {
    ...
    /* ---- create 2 vdatas --- */
    pid = VHstoredata (f, "PRES", (unsigned char*) pvals ,NP, DFNT_FLOAT32, 
                       "pressure vals",CLASS);
    if (pid  == -1) { printf(" VHstoredata store PRES err. "); }
    
    cid  = VHstoredatam(f, "COLOR",(unsigned char*) cvals, NC, DFNT_INT32, 
                        "rgb colors", CLASS, 3);
