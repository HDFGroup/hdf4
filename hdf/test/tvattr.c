...
write_vattrs(void)
{
    ...
    if (FAIL == VSfindex(vsid, FLDNAME1, &fldindex) || fldindex != 0) {
        num_errs++;
    ...
    if (FAIL == VSfindex(vsid, FLDNAME2, &fldindex) || fldindex != 1) {
        num_errs++;
    ...
    if (FAIL == VSfindex(vsid, FLDNAME1, &fldindex) || fldindex != 1) {
        num_errs++;
...
read_vattrs(void)
{
    ...
    if (FAIL == Vgetattr(vgid, 0, iattr1) || iattr1[0] != attr1[2] || iattr1[1] != attr1[3]) {
        num_errs++;
        printf(">>> Wrong values for attname1 of vgname0; ");
        printf("got %u %u, should be %u %u.\n", (unsigned)iattr1[0], (unsigned)iattr1[1], (unsigned)attr1[2],
               (unsigned)attr1[3]);
    }
    ...
    if (FAIL == Vgetattr(vgid, 1, iattr2) || iattr2[0] != attr2[0] || iattr2[1] != attr2[1]) {
        num_errs++;
    ...
    if (FAIL == VSgetattr(vsid, 0, 2, iattr3) || iattr3[0] != attr3[0] || iattr3[1] != attr3[1] ||
        iattr3[2] != attr3[2] || iattr3[3] != attr3[3] || iattr3[4] != attr3[4]) {
        num_errs++;
    ...
    /* use wrong findex, should fail */
    if (FAIL != VSattrinfo(vsid, 4, 0, NULL, &i_type, &i_count, &i_size)) {
        num_errs++;
    ...
    /* get the 2nd attr  */
    if ((FAIL == (iattrindex = VSfindattr(vsid, _HDF_VDATA, ATTNAME4))) || (iattrindex != 1)) {
        num_errs++;
    ...
    if ((FAIL == (iattrindex = VSfindattr(vsid, 0, VSNAME1))) || (iattrindex != 2)) {
        num_errs++;
