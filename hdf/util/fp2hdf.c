 ...
 *	Get the input data.
 */
int gdata(infile, in, strm, is_maxmin)
char *infile;
struct Input *in;
FILE *strm;
int *is_maxmin;
 ...
 */
int gmaxmin(infile, in, strm, is_maxmin)
char *infile;
struct Input *in;
FILE *strm;
int *is_maxmin;
 ...
 */
int gscale(infile, in, strm, is_scale)
char *infile;
struct Input *in;
FILE *strm;
int *is_scale;
	...
	if (in->rank == 2) {
		for (i = 0; i < im->vres; i++) {
			...
			for (j = 0; j < im->hres; j++) {
                ...
                if (pix > in->max) pix = in->max;       /* clip (bug fix) */
                if (pix < in->min) pix = in->min;       /* ditto */
