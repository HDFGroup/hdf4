              ...
              int32 *offsetarray, int32 *lengtharray)
{
    ...
    if ((offsetarray != NULL && lengtharray == NULL) || (offsetarray == NULL && lengtharray != NULL))
        HGOTO_ERROR(DFE_ARGS, FAIL);
        ...
        /* If the element is special, get the special info header and decode
           for special tag to detect compression/chunking/linked blocks */
            ...
            /* This is a chunked element, hand the task over to the chunking
                ...
                if (chk_coord != NULL)
                    count = HMCgetdatainfo(file_id, tag, ref, chk_coord, start_block, info_count, offsetarray,
                                           lengtharray);
                else /* BMR: check to see what should be done here */
                {
                    fprintf(stderr,
                            "\nERROR>>> Element with tag/ref %d/%d is a chunked element, the chunk's "
...
VSgetdatainfo(int32 vsid, uintn start_block, uintn info_count, int32 *offsetarray, int32 *lengtharray)
{
    ...
    if (access_rec->special == SPECIAL_LINKED) {
        ...
        /* Application only wants the offset and length of the blocks too */
            ...
            count = HDgetdatainfo(vs->f, VSDATATAG, vs->oref, NULL, start_block, info_count, offsetarray,
                                  lengtharray);
        }
        if (count == FAIL)
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
...
GRgetdatainfo(int32 riid, uintn start_block, uintn info_count, int32 *offsetarray, int32 *lengtharray)
{
    ...
    /* If the image already had a tag/ref pair, make sure it has actual data,
       if not, return 0 for info count */
        ...
        if ((offsetarray == NULL && lengtharray == NULL)) {
            count = HDgetdatainfo(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, NULL, start_block, 0, NULL,
                                  NULL);
            if (count == FAIL)
                HGOTO_ERROR(DFE_INTERNAL, FAIL);
        }
        ...
        /* Application requests offsets/lengths */
            ...
            count = HDgetdatainfo(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, NULL, start_block,
...
GRgetpalinfo(int32 gr_id, uintn pal_count, hdf_ddinfo_t *palinfo_array)
{
    ...
    while (ret_value != FAIL && idx < pal_count) {
        ...
        if (tag == DFTAG_IP8 || tag == DFTAG_LUT) { /* a palette tag is found */

            /* Get the palette's data info */
            ret_value = Hinquire(aid, NULL, &palinfo_array[idx].tag, &palinfo_array[idx].ref,
                                 &palinfo_array[idx].length, &palinfo_array[idx].offset, NULL, NULL, NULL);
            if (ret_value == FAIL)
                HGOTO_ERROR(DFE_INTERNAL, FAIL);
