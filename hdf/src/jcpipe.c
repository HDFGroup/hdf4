...
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; ci < cinfo->num_components; ci++) {
    ...
    (*cinfo->emethods->free_small) ((VOIDP) fullsize_data[1][ci]);
  }

  /* Free the top-level space */
  (*cinfo->emethods->free_small) ((VOIDP) fullsize_data[0]);
  (*cinfo->emethods->free_small) ((VOIDP) fullsize_data[1]);
}
...
#else
subsample (cinfo, fullsize_data, subsampled_data, fullsize_width, above,
        current, below, out)
compress_info_ptr cinfo;
