...
Get_oldimgs(int32 file_id, imginfo_t *img_info, uint16 searched_tag)
{
    ...
    while (Hfind(file_id, searched_tag, DFREF_WILDCARD, &find_tag, &find_ref, &find_off, &find_len,
                 DF_FORWARD) == SUCCEED) {
...
GRIget_image_list(int32 file_id, gr_info_t *gr_ptr)
{
    ...
    if ((img_info = (imginfo_t *)malloc((size_t)nimages * sizeof(imginfo_t))) == NULL)
    ...
    while (Hfind(file_id, DFTAG_RIG, DFREF_WILDCARD, &find_tag, &find_ref, &find_off, &find_len,
        ...
        while (DFdiget(group_id, &elt_tag, &elt_ref) != FAIL) { /* get next tag/ref */
    ...
    */
    ...
    for (i = 0; i < curr_image; i++) { /* go through the images looking for duplicates */
                ...
                if (img_info[i].img_ref == img_info[j].img_ref)
                    if (img_info[j].img_tag != DFTAG_NULL) {
                        ...
                        if (((img_info[i].offset != INVALID_OFFSET && img_info[i].offset != 0) &&
                            ...
                            (img_info[i].offset == 0 &&
                             (special_type == SPECIAL_LINKED || special_type == SPECIAL_CHUNKED))) {
                            ...
                            switch (img_info[i].img_tag) {
                                ...
                                case DFTAG_II8: /* Eldest style raster image, no grouping */
                                    if (img_info[j].img_tag != DFTAG_RI8 &&
                                        img_info[j].img_tag != DFTAG_CI8 && img_info[j].img_tag != DFTAG_II8)
        ...
        if (img_info[i].img_tag != DFTAG_NULL) {
            switch (img_info[i].grp_tag) {
                        ...
                        for (j = 0; j < Vntagrefs(img_key); j++) {
                            ...
                            switch (img_tag) {
                                ...
                                case DFTAG_LUT: /* Palette */
                                    ...
                                    /* Fill in some default palette dimension info, in case there isn't a
                                     * DFTAG_LD for this palette */
                                ...
                                case DFTAG_LD: /* Palette dimensions */
                                {
                                    ...
                                    /* read NT */
                                    if (Hgetelement(file_id, new_image->lut_dim.nt_tag,
                                                    new_image->lut_dim.nt_ref, ntstring) == FAIL)
                                        HGOTO_ERROR(DFE_READERROR, FAIL);
                                    ...
                                    if ((new_image->lut_dim.file_nt_subclass != DFNTF_HDFDEFAULT) &&
                                        ...
                                        (new_image->lut_dim.file_nt_subclass !=
                                         DFKgetPNSC(new_image->lut_dim.nt, DF_MT)))
                                        break; /* unknown subclass */
                                    if (new_image->lut_dim.file_nt_subclass !=
                                        DFNTF_HDFDEFAULT) { /* if native or little endian */
                                ...
                                case DFTAG_ID: /* Image description info */
                                {
                                    ...
                                    /* read NT */
                                    if (Hgetelement(file_id, new_image->img_dim.nt_tag,
                                                    new_image->img_dim.nt_ref, ntstring) == FAIL)
                                        HGOTO_ERROR(DFE_READERROR, FAIL);
                                    ...
                                    if ((new_image->img_dim.file_nt_subclass != DFNTF_HDFDEFAULT) &&
                                        ...
                                        (new_image->img_dim.file_nt_subclass !=
                                         DFKgetPNSC(new_image->img_dim.nt, DF_MT)))
                                        break; /* unknown subclass */
                                    if (new_image->img_dim.file_nt_subclass !=
                                        DFNTF_HDFDEFAULT) { /* if native or little endian */
                ...
                case DFTAG_RIG: /* Older style raster image, found in RIG */
                {
                    ...
                    while (DFdiget(GroupID, &elt_tag, &elt_ref) != FAIL) { /* get next tag/ref */
                        switch (elt_tag) {                                 /* process tag/ref */
                            ...
                            case DFTAG_LD: /* Palette dimensions */
                            {
                                ...
                                /* read NT */
                                if (Hgetelement(file_id, new_image->lut_dim.nt_tag, new_image->lut_dim.nt_ref,
                                                ntstring) == FAIL) {
                                    DFdifree(GroupID);
                                    HGOTO_ERROR(DFE_READERROR, FAIL);
                                }
                            ...
                            case DFTAG_ID: /* Image description info */
                            {
                                ...
                                /* read NT */
                                if (Hgetelement(file_id, new_image->img_dim.nt_tag, new_image->img_dim.nt_ref,
                                                ntstring) == FAIL) {
                                    DFdifree(GroupID);
                                    HGOTO_ERROR(DFE_GETELEM, FAIL);
                                }
    ...
    if (inil == outil) /* check for trivial input=output 'conversion' */
        ...
        switch (inil) {
            ...
            case MFGR_INTERLACE_COMPONENT:
                for (i = 0; i < ncomp; i++) {
                    in_comp_ptr[i] = ((const uint8 *)inbuf) +
                                     (size_t)i * (size_t)dims[YDIM] * (size_t)dims[XDIM] * comp_size;
                    in_pixel_add[i] = (int32)comp_size;
                    in_line_add[i]  = 0;
                } /* end for */
                break;
        ...
        switch (outil) {
            ...
            case MFGR_INTERLACE_COMPONENT:
                for (i = 0; i < ncomp; i++) {
                    out_comp_ptr[i] =
                        ((uint8 *)outbuf) + (size_t)i * (size_t)dims[YDIM] * (size_t)dims[XDIM] * comp_size;
                    out_pixel_add[i] = (int32)comp_size;
                    out_line_add[i]  = 0;
                } /* end for */
                break;
...
GRIupdatemeta(int32 hdf_file_id, ri_info_t *img_ptr)
{
    ...
    if (Hputelement(hdf_file_id, img_ptr->img_dim.nt_tag, img_ptr->img_dim.nt_ref, ntstring, (int32)4) ==
        FAIL)
    ...
    if (img_ptr->lut_ref != DFREF_WILDCARD) {
        ...
        ntstring[3] = DFNTC_BYTE;   /* class: data are numeric values */
        if (Hputelement(hdf_file_id, img_ptr->lut_dim.nt_tag, img_ptr->lut_dim.nt_ref, ntstring, (int32)4) ==
            FAIL)
            HGOTO_ERROR(DFE_PUTELEM, FAIL);
...
GRIupdateRIG(int32 hdf_file_id, ri_info_t *img_ptr)
{
    ...
    if (img_ptr->img_dim.nt != DFNT_UINT8 || (img_ptr->img_dim.ncomps != 1 && img_ptr->img_dim.ncomps != 3)) {
...
GRIupdateRI(int32 hdf_file_id, ri_info_t *img_ptr)
{
    ...
    if ((GroupID = Vattach(hdf_file_id, (img_ptr->ri_ref != DFREF_WILDCARD ? img_ptr->ri_ref : -1), "w")) ==
        FAIL)
...
GRIup_attr_data(int32 hdf_file_id, at_info_t *attr_ptr)
{
    ...
    {
        if ((attr_ptr->ref = (uint16)VHstoredata(hdf_file_id, attr_ptr->name, attr_ptr->data, attr_ptr->len,
                                                 attr_ptr->nt, RIGATTRNAME, RIGATTRCLASS)) == (uint16)FAIL)
                ...
                if (img_ptr->attr_modified == TRUE && img_ptr->lattr_count > 0) {
                    ...
                    while (t3 != NULL) {
                        ...
                        if (attr_ptr->new_at == TRUE) {
                            ...
                            if ((lGroupID = Vattach(gr_ptr->hdf_file_id, (int32)img_ptr->ri_ref, "w")) ==
                                FAIL)
                                HGOTO_ERROR(DFE_CANTATTACH, FAIL);
                        ...
                        if (NULL !=
                            (t3 = (void **)tbbtnext((TBBT_NODE *)t3))) /* get the next node in the tree */
                            attr_ptr = (at_info_t *)*t3; /* get actual pointer to the at_info_t */
                    }                                    /* end while */
...
GRcreate(int32 grid, const char *name, int32 ncomp, int32 nt, int32 il, int32 dimsizes[2])
{
    ...
    if (HAatom_group(grid) != GRIDGROUP || name == NULL || ncomp < 1 ||
        (il != MFGR_INTERLACE_PIXEL && il != MFGR_INTERLACE_LINE && il != MFGR_INTERLACE_COMPONENT) ||
...
GRwriteimage(int32 riid, int32 start[2], int32 in_stride[2], int32 count[2], void *data)
{
    ...
    size_t       pixel_mem_size;      /* size of a pixel in memory */
    size_t       pixel_disk_size;     /* size of a pixel on disk */
    ...
    else if (scheme == DFTAG_IMC) { /* coder no longer supported */
        HGOTO_ERROR(DFE_BADCODER, FAIL);
    }
    else {
        /* use lower-level routine to get the compression information */
        status =
            HCPgetcompinfo(ri_ptr->gr_ptr->hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, &comp_type, &cinfo);
        if (status == FAIL)
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
    }
    ...
    if (stride[XDIM] == 1 && stride[YDIM] == 1) { /* solid block of data */
        solid_block = TRUE;
        if ((start[XDIM] == 0 && start[YDIM] == 0) &&
            (count[XDIM] == ri_ptr->img_dim.xdim && count[YDIM] == ri_ptr->img_dim.ydim))
            whole_image = TRUE;
        else
            whole_image = FALSE;
    }    /* end if */
    ...
    pixel_mem_size =
        (size_t)(ri_ptr->img_dim.ncomps * DFKNTsize((ri_ptr->img_dim.nt | DFNT_NATIVE) & (~DFNT_LITEND)));
    pixel_disk_size = (size_t)(ri_ptr->img_dim.ncomps * DFKNTsize(ri_ptr->img_dim.nt));
    ...
    if (whole_image == TRUE) { /* write the whole image out */
        ...
        if (new_image == TRUE && ri_ptr->fill_img == TRUE) { /* create fill value arrays for various blocks */
            ...
            if (ri_ptr->fill_value != NULL) {
                DFKconvert(ri_ptr->fill_value, fill_pixel, ri_ptr->img_dim.nt, ri_ptr->img_dim.ncomps,
                           DFACC_WRITE, 0, 0);
            }    /* end if */
            ...
            if ((start[XDIM] + ((count[XDIM] - 1) * stride[XDIM]) + 1) < ri_ptr->img_dim.xdim)
                fill_hi_size =
                    (int32)pixel_disk_size *
                    (ri_ptr->img_dim.xdim - (start[XDIM] + ((count[XDIM] - 1) * stride[XDIM]) + 1));

            /* create the "line" pixel block */
            /* allocate space for the "line" block */
            fill_line_size = (int32)pixel_disk_size * ri_ptr->img_dim.xdim;
            if ((fill_line = malloc((size_t)fill_line_size)) == NULL)
                HGOTO_ERROR(DFE_NOSPACE, FAIL);
            HDmemfill(fill_line, fill_pixel, (uint32)pixel_disk_size, (uint32)ri_ptr->img_dim.xdim);
        ...
        if (solid_block == TRUE) { /* write out runs of data in the image */
            ...
            if (fill_image == TRUE) { /* surround the block to write with fill values */
                /* write out lines "below" the block */
                if (start[YDIM] > 0) { /* fill in the lines leading up the block */
                ...
                for (i = 0; i < count[YDIM]; i++) {
                    ...
                    /* at the same time. */
                    if ((fill_hi_size + fill_lo_size) > 0 && i < (count[YDIM] - 1))
                ...
                if ((start[YDIM] + ((count[YDIM] - 1) * stride[YDIM]) + 1) < ri_ptr->img_dim.ydim) {
                    for (i = start[YDIM] + ((count[YDIM] - 1) * stride[YDIM]) + 1; i < ri_ptr->img_dim.ydim;
                         i++)
                        if (Hwrite(ri_ptr->img_aid, fill_line_size, fill_line) == FAIL)
                            HGOTO_ERROR(DFE_WRITEERROR, FAIL);
                }  /* end if */
            }      /* end if */
            else { /* don't worry about fill values */
                for (i = 0; i < count[YDIM]; i++) {
            ...
            if (fill_image == TRUE) { /* create the "stride" pixel block */
                if (stride[XDIM] > 1) {
...
GRreadimage(int32 riid, int32 start[2], int32 in_stride[2], int32 count[2], void *data)
{
    ...
    /* Catch old images to avoid going into HCPgetcompinfo */
        ...
        /* use lower-level routine to get the compression information */
        status = HCPgetcompinfo(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, &comp_type, &cinfo);
        if (status == FAIL)
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
    }
    if (comp_type != COMP_CODE_NONE && comp_type != COMP_CODE_RLE &&
        comp_type != COMP_CODE_IMCOMP) { /* This is reading, why do we need encoder? -BMR */
        /* Check that the compression encoder is available */
        HCget_config_info(comp_type, &comp_config);
        if ((comp_config & (COMP_DECODER_ENABLED | COMP_ENCODER_ENABLED)) == 0) { /* coder not present?? */
            HGOTO_ERROR(DFE_BADCODER, FAIL);
        }
        if ((comp_config & COMP_DECODER_ENABLED) == 0) { /* decoder not present?? */
            HGOTO_ERROR(DFE_NOENCODER, FAIL);
        }
    }
    if (stride[XDIM] == 1 && stride[YDIM] == 1) { /* solid block of data */
    ...
    /* Get number-type and conversion information */
    platnumsubclass = (uint8)DFKgetPNSC(ri_ptr->img_dim.nt & (~DFNT_LITEND), DF_MT);
    convert         = (pixel_disk_size != pixel_mem_size) ||
              (ri_ptr->img_dim.file_nt_subclass != platnumsubclass); /* is conversion necessary? */
    ...
    if (image_data == FALSE) { /* Fake an image for the user by using the pixel fill value */
        void *fill_pixel;      /* converted value for the filled pixel */
        int32 at_index;
        ...
        if (whole_image == TRUE) { /* read the whole image in */
            /* Make certain we are at the beginning */
            if (Hseek(ri_ptr->img_aid, 0, DF_START) == FAIL)
                HGOTO_ERROR(DFE_SEEKERROR, FAIL);
        ...
        else {                /* read only part of the image in */
            int32 img_offset; /* current offset in the image data */
...
GRendaccess(int32 riid)
{
    ...
    if (ri_ptr->store_fill == TRUE) { /* store the fill value attribute before letting go of the image */
        if (GRsetattr(riid, FILL_ATTR, ri_ptr->img_dim.nt, ri_ptr->img_dim.ncomps, ri_ptr->fill_value) ==
            FAIL)
            HGOTO_ERROR(DFE_WRITEERROR, FAIL);
        ri_ptr->store_fill = FALSE;
    } /* end if */
...
GRreftoindex(int32 grid, uint16 ref)
{
    ...
    do {
        ri_ptr = (ri_info_t *)*t;
        if (ri_ptr != NULL && ((ri_ptr->ri_ref == ref) || (ri_ptr->ri_ref == DFREF_WILDCARD &&
                                                           ri_ptr->rig_ref == ref))) /* the ref matches */
            HGOTO_DONE(ri_ptr->index);
    } while ((t = (void **)tbbtnext((TBBT_NODE *)t)) != NULL);
...
--------------------------------------------------------------------------*/
int32
GRgetlutid(int32 riid, int32 lut_index)
{
    int32 ret_value = SUCCEED;
...
GRwritelut(int32 lutid, int32 ncomps, int32 nt, int32 il, int32 nentries, void *data)
{
    ...
    if (HAatom_group(lutid) != RIIDGROUP || ncomps < 1 || (DFKNTsize(nt) == FAIL) || nentries < 1 ||
        data == NULL)
        HGOTO_ERROR(DFE_ARGS, FAIL);
    ...
    if (ncomps == 3 && (nt == DFNT_UINT8 || nt == DFNT_UCHAR8) && il == MFGR_INTERLACE_PIXEL &&
        nentries == 256) {
        /* Check if LUT exists already */
        if (ri_ptr->lut_tag != DFTAG_NULL && ri_ptr->lut_ref != DFREF_WILDCARD) { /* LUT already exists */
            if (Hputelement(hdf_file_id, ri_ptr->lut_tag, ri_ptr->lut_ref, data,
                            ncomps * nentries * DFKNTsize(nt)) == FAIL)
                HGOTO_ERROR(DFE_PUTELEM, FAIL);
        }      /* end if */
        else { /* LUT does not exist */
    ...
    else { /* currently, we are not going to support non-standard palettes */
...
GRsetexternalfile(int32 riid, const char *filename, int32 offset)
{
    ...
    if ((tmp_aid = HXcreate(ri_ptr->gr_ptr->hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, filename, offset,
                            0)) == FAIL)
...
GRsetaccesstype(int32 riid, unsigned accesstype)
{
    ...
    /* check the validity of the RI ID */
    if (HAatom_group(riid) != RIIDGROUP ||
        (accesstype != DFACC_DEFAULT && accesstype != DFACC_SERIAL && accesstype != DFACC_PARALLEL))
        HGOTO_ERROR(DFE_ARGS, FAIL);
...
GRsetcompress(int32 riid, comp_coder_t comp_type, comp_info *cinfo)
{
    ...
    if ((comp_type < COMP_CODE_NONE || comp_type >= COMP_CODE_INVALID) && comp_type != COMP_CODE_JPEG)
        HGOTO_ERROR(DFE_ARGS, FAIL);
...
GRgetcomptype(int32 riid, comp_coder_t *comp_type)
{
    ...
    /* Use lower-level routine to get the new compression type */
        ...
        ret_value = HCPgetcomptype(file_id, ri_ptr->img_tag, ri_ptr->img_ref, &temp_comp_type);
        if (ret_value == FAIL)
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
...
GRsetattr(int32 id, const char *name, int32 attr_nt, int32 count, const void *data)
{
    ...
    if ((count > MAX_ORDER) || ((count * DFKNTsize(attr_nt)) > MAX_FIELD_SIZE))
        HGOTO_ERROR(DFE_ARGS, FAIL);
    ...
    if (found == TRUE) /* attribute already exists, just update it */
    {
        ...
        {                 /* not cacheable, write directly out to disk and throw away old in-memory copy */
            ...
            if (new_at_size > at_size || at_ptr->data == NULL) {
                free(at_ptr->data);
                if ((at_ptr->data = malloc((size_t)new_at_size)) == NULL)
                    HGOTO_ERROR(DFE_NOSPACE, FAIL);
            } /* end if */
            memcpy(at_ptr->data, data, (size_t)new_at_size);
    ...
    else                                  /* a new attribute */
    {
        ...
        if ((uint32)at_size < gr_ptr->attr_cache) { /* cacheable */
            /* allocate space for the attribute name & copy it */
            if ((at_ptr->data = (char *)malloc((size_t)at_size)) == NULL)
                HGOTO_ERROR(DFE_NOSPACE, FAIL);
            memcpy(at_ptr->data, data, (size_t)at_size);
            at_ptr->data_modified = TRUE;
            at_ptr->ref           = DFREF_WILDCARD;
        }      /* end if */
        else { /* non-cacheable */
            if ((at_ptr->ref = (uint16)VHstoredata(hdf_file_id, at_ptr->name, data, at_ptr->len, at_ptr->nt,
                                                   RIGATTRNAME, RIGATTRCLASS)) == (uint16)FAIL)
                HGOTO_ERROR(DFE_VSCANTCREATE, FAIL);
            at_ptr->data          = NULL;
            at_ptr->data_modified = FALSE;
        } /* end else */
...
GRgetattr(int32 id, int32 index, void *data)
{
    ...
    if (at_ptr->data == NULL) {
        ...
        if ((at_ptr->data = malloc((size_t)at_size)) == NULL)
            HGOTO_ERROR(DFE_NOSPACE, FAIL);
    ...
    memcpy(data, at_ptr->data, (size_t)at_size);
...
GRIgetaid(ri_info_t *ri_ptr, int acc_perm)
{
    ...
    if (ri_ptr->comp_img ||
        (ri_ptr->img_aid != 0 && (acc_perm & DFACC_WRITE) != 0 && (ri_ptr->acc_perm & DFACC_WRITE) == 0)) {
    ...
    if (ri_ptr->img_aid == 0) {
        /* Go get the AID */
        if (ri_ptr->comp_img) { /* Need to create the compressed data element */
            if ((ri_ptr->img_aid = HCcreate(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, COMP_MODEL_STDIO,
                                            &minfo, ri_ptr->comp_type, &(ri_ptr->cinfo))) == FAIL)
                HGOTO_ERROR(DFE_BADAID, FAIL);
            ...
            if (ri_ptr->use_cr_drvr) { /* Use compressed raster driver */
                ...
                if ((ri_ptr->img_aid =
                         HRPconvert(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, ri_ptr->img_dim.xdim,
                                    ri_ptr->img_dim.ydim, (int16)ri_ptr->img_dim.comp_tag, &(ri_ptr->cinfo),
                                    pixel_size)) == FAIL)
                    HGOTO_ERROR(DFE_BADAID, FAIL);
            }      /* end if */
            else { /* Use regular startaccess to create element */
                if ((ri_ptr->img_aid = Hstartaccess(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref,
                                                    (uint32)acc_perm)) == FAIL)
                    HGOTO_ERROR(DFE_BADAID, FAIL);
            }                       /* end else */
        }                           /* end else */
        if (ri_ptr->use_buf_drvr) { /* Convert to buffered special element if needed */
           ...
           int32         flags /* IN: flags */)
{
    ...
    if ((chunk[0].pdims = (DIM_DEF *)malloc((size_t)ndims * sizeof(DIM_DEF))) == NULL)
        HGOTO_ERROR(DFE_NOSPACE, FAIL);
    ...
    /* create correct disk version of fill pixel */
    if (ri_ptr->fill_value != NULL) {
        if (FAIL == DFKconvert(ri_ptr->fill_value, fill_pixel, ri_ptr->img_dim.nt, ri_ptr->img_dim.ncomps,
                               DFACC_WRITE, 0, 0))
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
    }    /* end if */
    else /* create default pixel fill value of all zero components */
    ...
    ret_value = HMCcreate(hdf_file_id,             /* HDF file handle */
                          ...
                          nlevels,                 /* nlevels */
                          (int32)pixel_disk_size,  /* fill value length */
               ...
               int32         *flags /* IN/OUT: flags */)
{
    ...
    if (ret_value != FAIL) {              /* make sure it is chunked element */
        if (special == SPECIAL_CHUNKED) { /* get info about chunked element */
            if ((ret_value = HDget_special_info(ri_ptr->img_aid, &info_block)) !=
                FAIL) { /* Does user want chunk lengths back? */
                if (chunk_def != NULL) {
             ...
             const void *datap /* IN: buffer for data */)
{
    ...
    int16           special;         /* Special code */
    size_t          csize;           /* physical chunk size */
    sp_info_block_t info_block;      /* special info block */
    ...
    ret_value = Hinquire(ri_ptr->img_aid, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &special);
    if (ret_value != FAIL) {
        if (special == SPECIAL_CHUNKED) { /* yes */
            ...
            if ((ret_value = HDget_special_info(ri_ptr->img_aid, &info_block)) != FAIL) {
                ...
                csize = 1;
                for (i = 0; i < info_block.ndims; i++)
                    csize *= (size_t)info_block.cdims[i];
                ...
                if (convert || switch_interlace == TRUE) {
                    /* Allocate space for the conversion buffer */
                    if ((img_data = malloc((size_t)(pixel_disk_size * csize))) == NULL)
                        HGOTO_ERROR(DFE_NOSPACE, FAIL);
            ...
            void  *datap /* IN/OUT: buffer for data */)
{
    ...
    if (comp_type != COMP_CODE_NONE && comp_type != COMP_CODE_RLE &&
        comp_type != COMP_CODE_IMCOMP) { /* BMR: this is reading, why do we need encoder? */
        /* Check that the compression encoder is available */
        HCget_config_info(comp_type, &comp_config);
        if ((comp_config & (COMP_DECODER_ENABLED | COMP_ENCODER_ENABLED)) == 0) {
            /* coder not present?? */
    ...
    if (ret_value != FAIL) {
        if (special == SPECIAL_CHUNKED) { /* yes */
            ...
            if ((ret_value = HDget_special_info(ri_ptr->img_aid, &info_block)) != FAIL) {
                ...
                /* Get the size of the pixels in memory and on disk */
                pixel_mem_size  = (size_t)(ri_ptr->img_dim.ncomps *
                                          DFKNTsize((ri_ptr->img_dim.nt | DFNT_NATIVE) & (~DFNT_LITEND)));
                pixel_disk_size = (size_t)(ri_ptr->img_dim.ncomps * DFKNTsize(ri_ptr->img_dim.nt));
                ...
                if (convert) {
                    ...
                    /* read it in */
                    if ((ret_value = HMCreadChunk(ri_ptr->img_aid, origin, img_data)) != FAIL) {
                        if (FAIL == DFKconvert(img_data, datap, ri_ptr->img_dim.nt,
                                               ri_ptr->img_dim.ncomps * (int32)csize, DFACC_READ, 0, 0))
                            HGOTO_ERROR(DFE_INTERNAL, FAIL);
                    ...
                    if ((ret_value = HMCreadChunk(ri_ptr->img_aid, origin, datap)) != FAIL)
                        ret_value = SUCCEED;
                }
...
GR2bmapped(int32 riid, int *tobe_mapped, int *name_generated)
{
    ...
    if (img_tag == DFTAG_RI8 || img_tag == DFTAG_CI8) {
        if (ri_ptr->img_dim.comp_tag == DFTAG_RLE || ri_ptr->img_dim.comp_tag == DFTAG_NULL)
            should_map = TRUE;
    }
