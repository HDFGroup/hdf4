                                ...
                                if (!HDstrcmp(textbuf,
                                    ...
                                    for (j = 0; j < Vntagrefs(img_key); j++) {
                                        ...
                                        if (img_tag == DFTAG_RI || img_tag == DFTAG_CI) {
                                            Store_imginfo(&img_info[curr_image], grp_tag, grp_ref, img_tag,
                                                          img_ref);
...
#ifdef OLD_WAY
            if (Hgetelement(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, (uint8 *)img_data) == FAIL)
                HGOTO_ERROR(DFE_GETELEM, FAIL);
#else                         /* OLD_WAY */
            /* Make certain we are at the beginning */
            if (Hseek(ri_ptr->img_aid, 0, DF_START) == FAIL)
    ...
    if (ri_ptr->img_aid == 0) {
        ...
        if (ri_ptr->comp_img) { /* Need to create the compressed data element */
            ...
            if (ri_ptr->use_cr_drvr) { /* Use compressed raster driver */
                ...
                if ((ri_ptr->img_aid =
                         Hstartaccess(hdf_file_id, ri_ptr->img_tag, ri_ptr->img_ref, acc_perm)) == FAIL)
                    HGOTO_ERROR(DFE_BADAID, FAIL);
            }                       /* end else */
        }                           /* end else */
