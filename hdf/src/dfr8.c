...
DFR8getimage(const char *filename, uint8 *image, int32 xdim, int32 ydim, uint8 *pal)
{
    ...
    if (Readrig.descimage.compr.tag) { /* compressed image */
        if (DFgetcomp(file_id, Readrig.image.tag, Readrig.image.ref, image, Readrig.descimage.xdim,
                      Readrig.descimage.ydim, Readrig.descimage.compr.tag) == FAIL)
            HGOTO_ERROR(DFE_INTERNAL, FAIL);
    }      /* end if */
...
DFR8Iputimage(const char *filename, const void *image, int32 xdim, int32 ydim, uint16 compress, int append)
{
    ...
    int    wdim                 = 0;    /* have dimensions already been written out? */
    int    ret_value            = SUCCEED;
    ...
    if (compress || CompressSet) {
        ...
        if (CompressSet == FALSE ||
            (compress > (uint16)1 && (int32)compress != CompType &&
        ...
        if (DFputcomp(file_id, DFTAG_CI, Writeref, image, xdim, ydim, pal, newpal, (int16)CompType,
                      &CompInfo) == FAIL)
    ...
    Writerig.aspectratio           = 1.0F;

    /* Write out Raster-8 tags for those who want it */
    if (CompType != DFTAG_GREYJPEG5) {
        r8tag = (uint16)(CompType ? ((CompType == DFTAG_RLE) ? DFTAG_CI8 : DFTAG_II8) : DFTAG_RI8);
    ...
    if (pal) {                 /* if there is a palette */
        ...
        if (Hdupdd(file_id, DFTAG_IP8, Writeref, Writerig.lut.tag, Writerig.lut.ref) == FAIL)
            HGOTO_ERROR(DFE_NOFREEDD, FAIL);
    } /* end if */
    ...
    while (DFdiget(GroupID, &elt_tag, &elt_ref) != FAIL) {
        ...
        switch (elt_tag) { /* process tag/ref */
            ...
            case DFTAG_ID: /* read description info */
                ...
                /* read NT */
                if (Hgetelement(file_id, rig->descimage.nt.tag, rig->descimage.nt.ref, ntstring) == FAIL) {
                    DFdifree(GroupID);
                    HGOTO_ERROR(DFE_GETELEM, FAIL);
                }
...
DFR8nimages(const char *filename)
{
    ...
    while (Hfind(file_id, DFTAG_RIG, DFREF_WILDCARD, &find_tag, &find_ref, &find_off, &find_len,
        ...
        while (!DFdiget(group_id, &elt_tag, &elt_ref)) { /* get next tag/ref */
            if (elt_tag == DFTAG_ID) { /* just look for ID tags to get the number of components */
                ...
                /* check for the image tag/ref */ if (elt_tag == DFTAG_CI ||
    ...
    /* go through the RI8s */
    find_tag = find_ref = 0;
    while (Hfind(file_id, DFTAG_RI8, DFREF_WILDCARD, &find_tag, &find_ref, &find_off, &find_len,
                 DF_FORWARD) == SUCCEED) {
    ...
    /* go through the CI8s */
    find_tag = find_ref = 0;
    while (Hfind(file_id, DFTAG_CI8, DFREF_WILDCARD, &find_tag, &find_ref, &find_off, &find_len,
                 DF_FORWARD) == SUCCEED) {
        img_off[curr_image] = find_off; /* store offset */
        curr_image++;
    } /* end while */
        ...
        do {
            if (Refset)
                ...
                if (!Readrig.image.ref)
                    ...
                    if (aid != FAIL && Hnextread(aid, DFTAG_RIG, DFREF_WILDCARD, DF_CURRENT) == FAIL) {
            ...
            /* RIG found */
            if (aid != FAIL) {
                Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, &ref, (int32 *)NULL, (int32 *)NULL,
    ...
    if (Refset || !foundRig) { /* No RIGs present, look for RI8 and CI8 */
        ...
        if (Refset)
            ...
            if (Readrig.image.ref) {
                ...
                if (aid != FAIL && Hnextread(aid, DFTAG_RI8, DFREF_WILDCARD, DF_CURRENT) == FAIL) {
                    Hendaccess(aid);
                    aid = FAIL;
                } /* end if */
            }     /* end if */
        ...
        if (aid != FAIL) {
            Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, &riref, (int32 *)NULL, (int32 *)NULL, (int32 *)NULL,
        ...
        if (Refset)
            ...
            if (Readrig.image.ref) {
                ...
                if (aid != FAIL && Hnextread(aid, DFTAG_CI8, DFREF_WILDCARD, DF_CURRENT) == FAIL) {
                    if (Hendaccess(aid) == FAIL)
                        HGOTO_ERROR(DFE_CANTENDACCESS, FAIL);
                    aid = FAIL;
                } /* end if */
        ...
        if (aid != FAIL) {
            Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, &ciref, (int32 *)NULL, (int32 *)NULL, (int32 *)NULL,
                     (int16 *)NULL, (int16 *)NULL);
            if (Hendaccess(aid) == FAIL)
                HGOTO_ERROR(DFE_CANTENDACCESS, FAIL);
        } /* end if */
