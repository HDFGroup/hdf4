...
const char *HDF_INTERNAL_VDS[] = {DIM_VALS,    DIM_VALS01,      _HDF_ATTRIBUTE, _HDF_SDSVAR,
       ...
       int32   asize /* IN: size of 'idarray' */)
{
    ...
    vgid = -1;
    while (FAIL != (vgid = Vgetid(f, vgid))) { /* until no more vgroups */
    ...
    for (i = 0; i < (int32)MAX_REF; i++) {
        if (lonevdata[i]) { /* insert into idarray up till asize */
            if (nlone < asize)
      ...
      int32   asize /* IN: size of 'idarray' */)
{
    ...
    nlone = 0;
    for (i = 0; i < (int32)MAX_REF; i++) {
        if (lonevg[i]) { /* insert into idarray up till asize */
            if (nlone < asize)
      ...
      const char *vgname /* IN: name of vgroup to find */)
{
    ...
    while (FAIL != (vgid = Vgetid(f, vgid))) { /* get instance of vgroup */
       ...
       const char *vsname /* IN: name of vdata to find */)
{
    ...
    while (FAIL != (vsid = VSgetid(f, vsid))) { /* get instance of vdata */
             ...
             uint16     *refarray /* IN/OUT: ref array to fill */)
{
    ...
    /* check if given id is a vgroup id */
        ...
        nactual_vds = 0; /* number of user-created vdatas to be retrieved */
        nfound_vds  = 0; /* number of user-created vdatas */
        for (ii = 0; ii < n_elements && ((nactual_vds < n_vds) || (n_vds == 0)) && nactual_vds <= nfound_vds;
             ii++) {
