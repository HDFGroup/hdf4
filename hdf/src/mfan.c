...
dumpentryKey(void *key, void *data)
{
    ...
    if (data != NULL) {
        node = (ANentry *)data;
        printf("ann_id=%ld, annref=%d, elmtag=%d, elmref=%d\n", (long)node->ann_id, node->annref,
               node->elmtag, node->elmref);
    }
    else
        printf("(NULL)\n");
} /* dumpentryKey() */
            ...
            intn   new_ann /* IN: new annotation flag */)
{
    ...
    *ann_key = AN_CREATE_KEY(type, ann_ref);
    ...
    if (FAIL == ann_entry->ann_id)
        HE_REPORT_GOTO("failed to insert annotation into ANIDGROUP atom group", FAIL);
                                         ...
                                         AN_FILE_DESC for file descriptions.*/)
{
    ...
    if ((nanns = Hnumber(an_id, ann_tag)) == 0) { /* ZERO annotations of 'type' in file */
        file_rec->an_num[type] = 0;
    ...
    for (i = 0; (i < nanns) && (more_anns != FAIL); i++) { /* see if annotation is there */
        if (FAIL == Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, &ann_ref, (int32 *)NULL, (int32 *)NULL,
                             (int32 *)NULL, (int16 *)NULL,
                             (int16 *)NULL)) { /* record what we found so far and return */
        ...
        if (type != AN_FILE_LABEL && type != AN_FILE_DESC) {
            if ((int32)FAIL == Hread(aid, (int32)4, datadi)) { /* record what we found so far and return */
                file_rec->an_num[type] = nanns;
                ret_value              = FAIL;
                goto done; /* we are done */
            }
...
ANIannlen(int32 ann_id /*  IN: annotation id */)
{
    ...
    if (ann_tag == DFTAG_DIL ||
        ann_tag == DFTAG_DIA) { /* 4=length of data tag/ref for data label or description */
        if ((ann_length = Hlength(file_id, ann_tag, ann_ref)) == FAIL)
            HE_REPORT_GOTO("Failed to find annotation length", FAIL);
        ann_length -= 4;
    }
    else if (ann_tag == DFTAG_FID ||
             ann_tag == DFTAG_FD) { /* not data tag/ref for file label or description */
        if ((ann_length = Hlength(file_id, ann_tag, ann_ref)) == FAIL)
            HE_REPORT_GOTO("Failed to find annotation length", FAIL);
    }
           ...
           int32 maxlen /* IN: size of space to return annotation in */)
{
    ...
    if (FAIL == Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, (uint16 *)NULL, &ann_len, (int32 *)NULL,
    ...
    if (ann_tag == DFTAG_FID || ann_tag == DFTAG_DIL) { /* Labels */
        if (ann_len > maxlen - 1)
            ann_len = maxlen - 1;
    }
    else { /* Descriptions */
    ...
    if (ann_tag == DFTAG_DIL || ann_tag == DFTAG_DIA) { /* Data label/description */
            ...
            int32       ann_len /* IN: length of annotation */)
{
    ...
    if (ann_tag == DFTAG_DIL ||
        ann_tag == DFTAG_DIA) { /* Data label/description
...
{ /* Switch on annotation type "atype" */
    ...
    switch ((ann_type)atype) {
        ...
        case AN_DATA_DESC:
            ...
            break;
        default:
            ann_tag = DFTAG_NULL; /*changed from 5 to DFTAG_NULL -BMR*/
    }                             /* switch */
    return ann_tag;
} /* ANatype2tag */
