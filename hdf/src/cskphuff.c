...
HCIcskphuff_encode(compinfo_t *info, int32 length, const uint8 *buf)
{
    ...
    while (length > 0) {           /* encode until we stored all the bytes */
        ...
        do { /* walk up the tree, pushing bits */
            stack[stack_ptr] =
                (skphuff_info->right[skphuff_info->skip_pos][skphuff_info->up[skphuff_info->skip_pos][a]] ==
                 a); /* push a 1 is this is the right node */
            stack_ptr++;
        ...
        do { /* output the bits we have */
            if (bit_count[stack_ptr] > 0) {
                if (Hbitwrite(info->aid, (intn)bit_count[stack_ptr], output_bits[stack_ptr]) !=
                    (intn)bit_count[stack_ptr])
                    HRETURN_ERROR(DFE_CENCODE, FAIL);
            } /* end if */
            stack_ptr--;
        } while (stack_ptr >= 0);
