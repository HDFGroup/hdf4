    ...
    while (!DFdiget(GroupID, &elt_tag, &elt_ref)) { /* get next tag/ref */
        switch (elt_tag) {                          /* process tag/ref */
            ...
            case DFTAG_LD:
                ...
                /* read NT */
                if (Hgetelement(file_id, rig->datadesc[type].nt.tag, rig->datadesc[type].nt.ref, ntstring) ==
                    FAIL) {
                    DFdifree(GroupID);
                    HGOTO_ERROR(DFE_READERROR, FAIL);
                }
...
DFGRaddrig(int32 file_id, uint16 ref, DFGRrig *rig)
{
    ...
    if (Ref.dims[IMAGE] == 0) {
        ...
        if (Hputelement(file_id, DFTAG_ID, ref, GRtbuf, (int32)(p - GRtbuf)) == FAIL)
            HGOTO_ERROR(DFE_PUTELEM, FAIL);
    ...
    if (!Ref.lut) {            /* associated lut not written to this file */
        ...
        if (Hputelement(file_id, DFTAG_LUT, ref, Grlutdata, (int32)lutsize) == FAIL)
            HGOTO_ERROR(DFE_PUTELEM, FAIL);
    ...
    if (Ref.dims[LUT] == 0) {
        ...
        if (Hputelement(file_id, DFTAG_LD, ref, GRtbuf, (int32)(p - GRtbuf)) == FAIL)
            HGOTO_ERROR(DFE_PUTELEM, FAIL);
    ...
    if ((Ref.dims[LUT] > 0) && (DFdiput(GroupID, DFTAG_LD, (uint16)Ref.dims[LUT]) == FAIL))
        HGOTO_ERROR(DFE_PUTGROUP, FAIL);

    if ((Ref.lut > 0) && (DFdiput(GroupID, rig->data[LUT].tag, rig->data[LUT].ref) == FAIL))
        HGOTO_ERROR(DFE_PUTGROUP, FAIL);
...
DFGRIriginfo(int32 file_id)
{
    ...
    for (i = 0; i < 4; i++) { /* repeat for RIG, RI8, CI8, II8 */
        if (isfirst) {
            ...
            if ((aid != FAIL) && Hnextread(aid, gettag, getref, DF_CURRENT) == FAIL) {
                Hendaccess(aid);
                aid = FAIL;
            }
        }
              ...
              int *compressed, uint16 *compr_type, int *has_pal)
{
    ...
    if ((xdim != Grread.datadesc[type].xdim) || (ydim != Grread.datadesc[type].ydim))
        HGOTO_ERROR(DFE_ARGS, FAIL);
    ...
    if (Grread.datadesc[type].compr.tag) { /* compressed image/lut */
        ...
        *compr_type = Grread.datadesc[type].compr.tag;
        if ((Grreqil[type] >= 0) && (Grreqil[type] != Grread.datadesc[type].interlace))
            HGOTO_ERROR(DFE_UNSUPPORTED, FAIL);
        if (DFgetcomp(file_id, Grread.data[type].tag, Grread.data[type].ref, (uint8 *)imlut,
                      Grread.datadesc[type].xdim, Grread.datadesc[type].ydim,
        ...
        *compressed = 0;
        if (Grreqil[type] >= 0) {
            if (Grreqil[type] >= Grread.datadesc[type].ncomponents) {
                ...
                aid = Hstartread(file_id, Grread.data[type].tag, Grread.data[type].ref);
                if (aid == FAIL)
                    HGOTO_ERROR(DFE_BADAID, FAIL);
                ...
                for (i = 0; i < Grread.datadesc[type].xdim; i++) {
                    ...
                    for (j = 0; j < bufsize; j++) {
                        destp = (uint8 *)imlut + destsize[0] * currpos[0] + destsize[1] * currpos[1] +
        ...
        if (Hgetelement(file_id, Grread.data[type].tag, Grread.data[type].ref, (uint8 *)imlut) == FAIL) {
            *has_pal = 0;
            HGOTO_ERROR(DFE_GETELEM, FAIL);
        }
        else
              ...
              int newfile)
{
    ...
    if ((Ref.dims[type] == 0 &&
         (xdim != Grwrite.datadesc[type].xdim || ydim != Grwrite.datadesc[type].ydim)) ||
        !imlut)
        HGOTO_ERROR(DFE_ARGS, FAIL);
    ...
    if (is8bit) {
        wtag = (uint16)((type == LUT) ? DFTAG_IP8
                        : Grcompr     ? ((Grcompr == DFTAG_RLE) ? DFTAG_CI8 : DFTAG_II8)
                                      : DFTAG_RI8);
    ...
    if (is8bit) {
        ...
        if ((Ref.lut >= 0) && Hdupdd(file_id, DFTAG_IP8, wref, DFTAG_LUT, wref) == FAIL)
            HGOTO_ERROR(DFE_DUPDD, FAIL);
        p = (uint8 *)&r8dims.xdim;
