...
VOID
emit_marker(int32 aid, JPEG_MARKER mark)
/* Emit a marker code */
{
    emit_byte(aid, 0xFF);
    emit_byte(aid, mark);
...
LOCAL       VOID
emit_2bytes(int32 aid, int value)
...
LOCAL int
emit_dqt(compress_info_ptr cinfo, int32 aid, int index)
/* Emit a DQT marker */
...
GLOBAL      VOID
jselwhdf(compress_info_ptr cinfo)
{
