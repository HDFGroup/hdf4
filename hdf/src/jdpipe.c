...
/* Release a sample image created by alloc_sampimage */
{
  ...
  (*cinfo->emethods->free_small) ((VOIDP) image);
}
...
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    (*cinfo->emethods->free_small) ((VOIDP) subsampled_data[1][ci]);
  }

  /* Free the top-level space */
  (*cinfo->emethods->free_small) ((VOIDP) subsampled_data[0]);
  (*cinfo->emethods->free_small) ((VOIDP) subsampled_data[1]);
}
...
#else
expand (cinfo, subsampled_data, fullsize_data, fullsize_width, above, current,
        below, out)
decompress_info_ptr cinfo;
