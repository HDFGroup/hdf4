    ...
    switch (scheme) {
        case DFTAG_RLE:                               /* RLE compression (8-bit or 24-bit(?) images */
            ...
            if (buftype == 2) {
                ...
                num_blocks   = (ydim > (int32)R8_MAX_BLOCKS) ? (int32)R8_MAX_BLOCKS : ydim;
                block_length = (xdim > (int32)R8_MAX_LENGTH) ? (int32)R8_MAX_LENGTH : xdim;
...
DFgetcomp(int32 file_id, uint16 tag, uint16 ref, uint8 *image, int32 xdim, int32 ydim, uint16 scheme)
{
    ...
    if (Hinquire(aid, (int32 *)NULL, (uint16 *)NULL, (uint16 *)NULL, &cisize, (int32 *)NULL, (int32 *)NULL,
