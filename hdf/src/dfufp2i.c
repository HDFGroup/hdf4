...
int
DFUfptoimage(int32 hdim, int32 vdim, float max, float min,
             float *hscale, float *vscale, float *data, uint8 *palette,
...
process(Input * in, Output * out)
{
     ...
     */
    if (!in->is_hscale)
        in->hscale = (float *) HDmalloc((uint32) (1 + in->hdim) * sizeof(float));
    if (!in->is_vscale)
        in->vscale = (float *) HDmalloc((uint32) (1 + in->vdim) * sizeof(float));
...
{
    int j, theval;
    float    *f, *dxs, *dys, *xv, *yv, *lim, delx, dely, pt, xrange, yrange,
    ...
    dxs = (float *) HDmalloc(sizeof(float) * (size_t)out->hres);
    /* temp space for dx's */
    dys = (float *) HDmalloc(sizeof(float) * (size_t)out->vres);
    /* temp space for dy's */
...
compute_offsets(float *scale, int32 dim, int32 *offsets, int32 res)
{
    ...
    midpt = (float *) HDmalloc(sizeof(float) * (size_t)dim);
