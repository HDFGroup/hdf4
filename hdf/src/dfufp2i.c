...
process(Input *in, Output *out)
{
    ...
    ret = DFR8addimage(out->outfile, (char *)out->image, out->hres, out->vres, (uint16)out->compress);
    if (ret < 0)
        return ret;
    /*
...
pixrep_scaled(Input *in, Output *out)
{
    ...
    voffsets = (int32 *)malloc((uint32)(out->vres + 1) * sizeof(int32));
    pixvals  = (uint8 *)malloc((uint32)in->hdim + 1);
